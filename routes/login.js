...
const addUserToDatabase = (user, db) => {
  ...
  return userExists(user,db).then((res)=>{
    if (res){
      ...
      return db.query(`INSERT INTO users ( email, name, password, organization_id, admin)
      VALUES ($1,$2,$3,$4,$5) Returning *; `, [ user.email, user.name, user.password, user.organization_id, user.admin])
        .then((res) => {
         return res.rows[0]
          console.log('res', res) ;
        })
